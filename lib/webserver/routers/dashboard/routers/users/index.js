...
const pageAddWhitelistSubmit = async (req, res) => {
  ...
  try {
    ...
    logger.info(`${MODULE_NAME} EE5F1183: User hostname whitelist added`, {
      xid,
      email,
      hostname,
    });
...
const pageRemoveWhitelistSubmit = async (req, res) => {
  const { xid, currentUser } = res.locals;
  const { email, hostname } = req.params;
  ...
  try {
    ...
    req.flash('info', `Hostname "${hostname}" remove from whitelist.`);

    logger.info(`${MODULE_NAME} 0CA7CFFA: Whitelist removed`, {
      xid,
      email,
      hostname,
    });
  } catch (e) {
    req.flash('warn', `Failed to remove "${hostname}" from whitelist.`);
  } finally {
    res.redirect(urlJoin(
      req.baseUrl,
      '/view',
...
router.get('/remove-whitelist/:email/:hostname', pageRemoveWhitelistSubmit);
