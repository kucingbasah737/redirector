...
const getHitsPerDay = require('../../../../get-hits-per-day');
...
const pageMain = async (req, res) => {
  ...
  try {
    ...
    const hitsPerDay = await getHitsPerDay(xid, null, emailSelector, 31, true);
...
const pageView = async (req, res) => {
  ...
  try {
    ...
    const hitsPerDay = await getHitsPerDay(xid, target.uuid, null, 31, true);
