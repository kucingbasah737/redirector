...
const pageHitCount = async (req, res) => {
  const { xid, currentUser } = res.locals;
  const { selector, hostname, name } = req.params;
  ...
  try {
    ...
    const value = hitCount[0]?.value;
    res.end((value || -9999).toString());
  } catch (e) {
...
router.get('/hit-count/:selector/:hostname/:name', pageHitCount);
