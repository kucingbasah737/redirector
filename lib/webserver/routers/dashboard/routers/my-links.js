...
const pageMain = async (req, res) => {
  ...
  try {
    ...
    const hitsPerDay = await getHitsDaily(xid, null, emailSelector, 31, true);
    const hitsPerHour = await getHitsHourly(xid, null, emailSelector, 24, true);
...
const pageView = async (req, res) => {
  ...
  try {
    ...
    const hitsPerDay = await getHitsDaily(xid, target.uuid, null, 31, true);
    const hitsPerHour = await getHitsHourly(xid, target.uuid, null, 24, true);
...
const pageUpdateSubmit = async (req, res) => {
  ...
  try {
     ...
     */
    const newValues = {
      user_email: (currentUser.super && email) || target.email,
